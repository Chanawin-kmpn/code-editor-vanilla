...
interface ConstructSnippetProps {
	id: string;
	codeMap: {
		css?: string;
		javascript?: string;
...
function constructSnippet({
    ...
    <head>
      ...
      <title>ซีเอสๆ iFrame playground</title>
...
export function wrapForReact(
	...
	try {
		const match = code!.match(renderExpression);
		if (!match) {
			throw new Error('Render expression not found in the code.');
		}
		const [, renderedContent] = match;
